...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected XMD and zXMD supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zXMDsupply']:
                assert_equal(node['zXMDsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check XMD and zXMD supply at the beginning
        # ------------------------------------------
        # zXMD supply: 2 coins for each denomination
        ...
        # XMD supply: block rewards minus burned fees for minting
        ...
        # Verify that XMD and zXMD supplies were properly updated after the spends and reorgs
        self.log.info("Check XMD and zXMD supply...")
