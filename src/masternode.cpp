...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if(nHeight >  50000) return  12500 * COIN;
...
{
    if(nHeight > 5000000) return       10 * COIN;
    if(nHeight > 1400000) return       20 * COIN;
    if(nHeight > 1300000) return       30 * COIN;
    if(nHeight > 1200000) return       40 * COIN;
    if(nHeight > 1100000) return       50 * COIN;
    if(nHeight > 1000000) return       60 * COIN;
    if(nHeight >  900000) return       70 * COIN;
    if(nHeight >  500000) return       80 * COIN;
    if(nHeight >  400000) return       75 * COIN;
    if(nHeight >  300000) return       70 * COIN;
    if(nHeight >  200000) return       50 * COIN;
    if(nHeight >  100000) return       40 * COIN;
    if(nHeight >   50000) return       30 * COIN;
    if(nHeight >    5000) return       25 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 XMD tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 XMD tx -> 1 confirmation
