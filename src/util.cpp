...
const char * const PIVX_CONF_FILENAME = "mandike.conf";
const char * const PIVX_PID_FILENAME = "mandike.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Mandike only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\mandike
// Windows >= Vista: C:\Users\Username\AppData\Roaming\mandike
// Mac: ~/Library/Application Support/mandike
// Unix: ~/.mandike
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mandike";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\mandikeParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\mandikeParams
    // Mac: ~/Library/Application Support/mandikeParams
    // Unix: ~/.mandike-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MandikeParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "MandikeParams";
#else
    // Unix
    return pathRet / ".mandike-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty mandike.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Mandike config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override mandike.conf
